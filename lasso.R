...
plot(JN_model_high)
cv1=cv.lars(JN_high_x,JN_high_y,type="lar")
step=cv1$index[which.min(cv1$cv)] #the number of steps in lars procedure
JN_model_high$beta[step,]  #best coefficients

JN_model_low=lars(JN_low_x,JN_low_y,type="lasso")
JN_model_low
summary(JN_model_low)
plot(JN_model_low)
cv2=cv.lars(JN_low_x,JN_low_y,type="lar")
step=cv2$index[which.min(cv2$cv)] #the number of steps in lars procedure
JN_model_low$beta[step,] #best coefficients

SW_model_high=lars(SW_high_x,SW_high_y,type="lar")
SW_model_high
summary(SW_model_high)
plot(SW_model_high)
cv3=cv.lars(SW_high_x,SW_high_y,type="lar")
step=cv3$index[which.min(cv3$cv)] 
SW_model_high$beta[step,]

SW_model_low=lars(SW_low_x,SW_low_y,type="lar")
SW_model_low
summary(SW_model_low)
plot(SW_model_low)
cv4=cv.lars(SW_low_x,SW_low_y,type="lar")
step=cv4$index[which.min(cv4$cv)] 
SW_model_low$beta[step,]